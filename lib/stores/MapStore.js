...
var MapActions_1 = require("../actions/MapActions");
function makeid() {
    var text = "";
...
var MapStore = /** @class */ (function (_super) {
    ...
    function MapStore(dispatcher) {
        ...
        var onDispatch = function (action) {
            if (action instanceof MapActions_1.LoadMapEvent) {
                _this.state = action.payload.map;
                _this.emitChange();
            }
            else if (action instanceof MapActions_1.NewNodeIntentEvent) {
            ...
            else if (action instanceof MapActions_1.NodeDraggedEvent) {
            ...
            else if (action instanceof MapActions_1.InitiateNodeDeletionEvent) {
                var id_1 = action.payload.id;
                _this.state.nodes = _this.state.nodes.filter(function (node) { return node.id !== id_1; });
                _this.emitChange();
            }
            else if (action instanceof MapActions_1.InitiateConnection) {
                var found = false;
            ...
            else if (action instanceof MapActions_1.InitiateConnectionDeletion) {
            ...
            else if (action instanceof MapActions_1.InitiateConnectionEdit) {
...
var mapStoreInstance = new MapStore(MapEditorDispatcher_1.MapEditorDispatcher);
exports["default"] = mapStoreInstance;
